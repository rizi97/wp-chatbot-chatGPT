jQuery(document).ready(function($) {
    ...
    function sendMessage() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                const final_response = document.createElement('div');
                final_response.classList.add('final_response');

                chatboxMessages.appendChild(final_response);

                // chatboxMessages.appendChild(container);
                
                new Typed(final_response, {
                    strings: [container.innerHTML],
                    typeSpeed: 2,
                    startDelay: 100,
                    contentType: 'html', // Render HTML content
                    showCursor: false,
                    onComplete: function() {
                        final_response.scrollTop = final_response.scrollHeight;
                    }
                });
    ...
    function autoResize() {
        chatInput.css('height', 'auto');  // Reset height to auto to allow shrinking
